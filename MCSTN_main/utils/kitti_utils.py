...
def getlabelPC(PC, box, offset=0, scale=1.0):
    ...
    return new_label
# 第一次过滤：this_PC = kitti_utils.cropPC(this_PC, this_BB, offset=10, scale=1.0) 从整张点云过滤出经过偏移bbox内的点
...
# train：
# self.model_PC[i] = kitti_utils.getModel(PCs, BBs, offset=0, scale=1.25)第一次从原始轨迹数据种拿model
# 第二次：根据前一帧和第一帧生成模板点云，注意这里对前一阵的GT bbox做了一个偏移 [fir_PC, pre_PC], [fir_BB, sample_pre_BB]
...
def cropAndCenterPC_withClaAndOff(PC, cut_box, gt_box, offset, scale, limit_area=None):
    ...
    class_label = getlabelPC(new_PC, gt_box, scale=np.array([1.10, 1.05, 1.025]))  # 在gt框内的点标记为1 不在的标记为0

    rot_mat = np.transpose(new_box_cut.rotation_matrix)
    trans = -new_box_cut.center
    
    # align data  对齐到cut_box中，以该bbox中心为0
    new_PC.translate(trans)
...
'''
train:
    pc:得到的是 limit_area内部的点；3×N
    class_label:点的label信息
    offcenter:gt_BB和sample_cur_BB的偏差（中心偏差和旋转）
    subsample:采样的点个数
'''
def subsamplePC_withClaAndOff(PC, class_label, offcenter, subsample_number, cut=False, istrain=True):
    if np.shape(PC)[1] > 2:
        ...
        PC = PC.reshape((3, subsample_number))
        offcenter = np.tile(offcenter, [np.size(class_label), 1])   # 复制offcenter到(1024,4)
    else:
        PC = np.zeros((3, subsample_number))
...
# self.model_PC[i] = kitti_utils.getModel(PCs, BBs, offset=0, scale=1.25)第一次从原始轨迹数据种拿model
# 第二次：根据前一帧和第一帧生成模板点云，注意这里对前一阵的GT bbox做了一个偏移 [fir_PC, pre_PC], [fir_BB, sample_pre_BB]
def getModel(PCs, boxes, offset=0, scale=1.0, normalize=False,limit_area=None):
    ...
    for PC, box in zip(PCs, boxes):
        cropped_PC = cropAndCenterPC(
            PC, box, offset=offset, scale=scale, normalize=normalize,limit_area=limit_area)
        # try:
