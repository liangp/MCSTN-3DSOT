...
def sparse_cropPC(PC, box, limit_area=None):
    # boxTemp = copy.deepcopy(box)
    maxi = np.max(box, 1)
    mini = np.min(box, 1)

    x_filt_max = PC[0, :] < maxi[0]
    x_filt_min = PC[0, :] > mini[0]
    y_filt_max = PC[1, :] < maxi[1]
    y_filt_min = PC[1, :] > mini[1]
    z_filt_max = PC[2, :] < maxi[2]
    z_filt_min = PC[2, :] > mini[2]
        ...
        for PCs, BBs, list_of_anno in batch: 
            ...
            for i, _ in enumerate(PCs):
                ...
                if i == 0:
                    ...
                    target_PC = kitti_utils.cropAndCenterPC(cur_PC, pre_BB, offset=dataset.offset_BB, scale=1.25, limit_area=area_extents)
                    model_PC = kitti_utils.getModel([PCs[0], PCs[i-1]], [results_BBs[0], results_BBs[i-1]], scale=1.25, limit_area=area_extents)
                    ...
                    pred_hm, pred_loc, pred_z_axis = model(model_PC.unsqueeze(0).cuda(),
                                                           target_PC.unsqueeze(0).cuda())

                    num_block = len(pred_hm)

                    pred_hm = pred_hm[num_block-1]
                    pred_loc = pred_loc[num_block-1]
                    pred_z_axis = pred_z_axis[num_block-1]
...
def test_model_waymo_format(opts, model, dataset, batch_id):
    ...
    for i in range(len(PCs)):
        if i == 0:
            ...
            if opts.visual:
                visual_data['pointcloud'].append(cur_PC.points)
                visual_data['gt_box'].append(BBs[i].corners())
                visual_data['pred_box'].append(results_BBs[-1].corners())
        ...
        # for visualization
        if opts.visual:
            save_path = "visualization/data/%s_%s_%d.pth" % (
            "waymo", "pedestrian", batch_id)
            file = open(save_path, "wb")
            pickle.dump(visual_data, file)
            file.close()
    return Success_main.average, Precision_main.average