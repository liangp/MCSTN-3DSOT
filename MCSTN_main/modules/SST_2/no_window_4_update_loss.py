...
class cross_attention(nn.Module):
    ...
    def forward(self, search_feat, search_coors, search_mask, template_feat, template_coors, template_mask):
        ...
        '''
        bs = search_feat.size(0)
        #template_xyz = torch.as_tensor(template_coors, dtype=template_feat.dtype)
...
class no_window_block(nn.Module):
    def __init__(self,
        ...
        # self.self_attention2 = self_attention(d_model, nhead)
    ...
    def forward(self, search_feat, search_xyz, search_mask, template_feat, template_xyz, template_mask, search_original_indx_list, num_search, template_original_indx_list, num_template):
        ...
        self_search = self.self_attention1(search_feat, search_xyz, search_mask)  # [B,512,C]
        self_template = self.self_attention1(template_feat, template_xyz, template_mask)  # [B,256,C]
...
class Direct_attenttion10(nn.Module):
    ...
    def __init__(
            self,
            d_model=[],  # d_model=[128,] * 6,
            nhead=[],  # nhead=[8, ] * 6,
            num_blocks=1,
            dim_feedforward=[],  # dim_feedforward=[256, ] * 6,
            dropout=0.0,
            activation="gelu",  # gelu比用relu要强一点
            output_shape=[32, 32],  # shape of output bev feature  output_shape=[468, 468],
            num_attached_conv=3,  # 网络Head前面多加了两个 3*3 Convolution  num_attached_conv=3,
            conv_in_channel=64,  # 128
            conv_out_channel=64,  # 128
        ...
        self.d_model = d_model  # [128, 128, 128, 128, 128, 128]
        self.nhead = nhead  # [8, 8, 8, 8, 8, 8]
        self.checkpoint_blocks = checkpoint_blocks  # []
        self.conv_shortcut = conv_shortcut  # False
        ...
        self.output_shape = output_shape  # [468, 468]

        self.debug = debug  # True

        self.num_attached_conv = num_attached_conv  # 3
        ...
        if num_attached_conv > 0:
            ...
            for i in range(num_attached_conv):
                ...
                # Conv2d(128, 128, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
                # Conv2d(128, 128, kernel_size=(3, 3), stride=(1, 1), padding=(2, 2), dilation=(2, 2), bias=False)
    ...
    def forward(self, search_feat0, search_coors, template_feat0, template_coors, train_test):
        ...
        for i, block in enumerate(self.block_list_search):
            ...
            search_feat = self.single_padding_tensor(search_feat, search_coors)  # N1,C->B,N2,C
            paddng_search.append(search_feat)
#            template_feat = sum(output_template_list)
#            template_feat = self.single_padding_tensor_template(template_feat, template_coors)
#            template_feat = self.single_padding_tensor_template(self_2d_template, template_coors)
...
#                        temp = conv(output_search)
#                        if temp.shape == output_search.shape and self.conv_shortcut:
#                            output_search = temp + output_search
#                        else:
#                            output_search = temp
#                ouput_feat_loss.append(output_search)
...
#            ouput_feat_loss.append(output_search)
    ...
    def recover_bev(self, voxel_feat, coors, batch_size):
        ...
        '''
        ny, nx = self.output_shape  # [468, 468]
        ...
        for batch_itt in range(batch_size):
            ...
            batch_mask = coors[:, 0] == batch_itt  # 得到第一个sample的mask信息
            this_coors = coors[batch_mask, :]  # 得到第一个sample的coors
            ...
            padding_mask[indices] = 0  # 0表示没有填充
        ...
        padding_masks = padding_masks.view(batch_size, nx*ny) == -1  # -1表示填充的
