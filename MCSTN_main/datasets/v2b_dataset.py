...
class TrainDataset(BaseDataset):
        ...
        if (not self.preload_data_path is None) and os.path.exists(self.preload_data_path + '_PCs.pth'):
            ...
            for tracklet in tqdm(self.list_of_tracklet_anno, ncols=opts.ncols, \
                ...
                for idx in range(len(tracklet)):
                    this_PC, this_BB = self.getBBandPC(tracklet[idx])  # 当前帧的雷达点和GT框
    ...
    def getitem(self, index):
        ...
        '''every anno include:
                            ...
                            "height", "width", "length", "x", "y", "z", "rotation_y"
                            '''
        this_anno = self.list_of_anno[anno_idx]  # 得到注释的索引
        
        # 1. Get the search point cloud
        cur_PC, cur_BB = self.getPCandBBfromIndex(anno_idx)  # 当前帧的点云以及GT框
        ...
        model_PC = kitti_utils.getModel([fir_PC, pre_PC], [fir_BB, sample_pre_BB], scale=1.25, limit_area=self.area_extents)
        ...
        corners = adapt_cur_BB.corners()[:2].T
