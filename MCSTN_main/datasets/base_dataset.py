...
class kittiDataset():
    ...
    def getPCandBBfromPandas(self, box, calib):
        center = [box["x"], box["y"] - box["height"] / 2, box["z"]]  # GT框的3d中心坐标
...
class BaseDataset(Dataset):
    def __init__(self, which_dataset, path, split, category_name="Car", offset_BB=np.zeros(1), scale_BB=np.ones(1)):
        ...
        self.sceneID = self.dataset.getSceneID(split=split)  # train 17个
        ...
        self.list_of_tracklet_anno = self.dataset.getListOfAnno(self.sceneID, category_name)  # 生成每个目标的轨迹信息
